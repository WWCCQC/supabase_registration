// components/tables/TechnicianCountTable.tsx
"use client";
...
interface TechnicianCountTableProps {
  data: PivotData[];
  loading?: boolean;
}

export default function TechnicianCountTable({ data, loading = false }: TechnicianCountTableProps) {
  if (loading) {
    return (
  ...
  rsms.forEach(rsm => {
    ...
    providers.forEach(provider => {
      workTypes.forEach(workType => {
        pivotData[rsm][`${provider}_${workType}`] = 0;
      });
    });
  });
  ...
  data.forEach(item => {
    ...
    if (pivotData[item.RBM]?.[key] !== undefined) {
  ...
  rsms.forEach(rsm => {
    ...
    providers.forEach(provider => {
      rsmTotals[rsm][provider] =
        (pivotData[rsm][`${provider}_Installation`] || 0) +
        (pivotData[rsm][`${provider}_Repair`] || 0);
    });
  });
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', fontFamily: 'Arial, sans-serif' }}>
        ...
        <tbody>
          ...
          <tr style={{ backgroundColor: '#f3f4f6' }}>
            ...
            <td style={totalCellStyle}>{fmt(grandTotal)}</td>
              ...
              <tr key={rsm}>
                ...
                {providers.map(provider => (
                  <React.Fragment key={provider}>
                    ...
                    <td style={cellStyle}>{fmt(pivotData[rsm][`${provider}_Installation`] || 0)}</td>
                    <td style={cellStyle}>{fmt(pivotData[rsm][`${provider}_Repair`] || 0)}</td>
                  </React.Fragment>
                ))}
              </tr>
            );
