...
export async function GET(req: Request) {
  try {
    ...
    let qWT: any = supabase.from("technicians").select("work_type").not("work_type", "is", null).limit(5000);
    ...
    for (const raw of Array.from(new Set((listWT || []).map((r: any) => r.work_type).filter(Boolean)))) {
      ...
      let q: any = supabase.from("technicians").select("*", { count: "exact", head: true }).ilike("work_type", `%${key}%`);
    ...
    let qPv: any = supabase.from("technicians").select("provider").not("provider", "is", null).limit(5000);
    ...
    for (const raw of Array.from(new Set((listPv || []).map((r: any) => r.provider).filter(Boolean)))) {
      const key = String(raw).trim();
      let q: any = supabase.from("technicians").select("*", { count: "exact", head: true }).ilike("provider", `%${key}%`);
      q = applyFilters(q, params);
      q = await applyDateFilterSafe(q, params);
      const { count = 0 } = await q;
