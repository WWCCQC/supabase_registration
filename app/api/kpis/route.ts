...
export async function GET(req: Request) {
  try {
    ...
    for (const raw of Array.from(new Set((listWT || []).map((r: any) => r.work_type).filter(Boolean)))) {
      ...
      const { count } = await q;
      by_work_type.push({ key, count: count || 0, percent: total ? +(100 * (count || 0) / total).toFixed(2) : 0 });
    }
    ...
    for (const raw of Array.from(new Set((listPv || []).map((r: any) => r.provider).filter(Boolean)))) {
      ...
      q = await applyDateFilterSafe(q, params);
      const { count } = await q;
      by_provider.push({ key, count: count || 0, percent: total ? +(100 * (count || 0) / total).toFixed(2) : 0 });
    }
